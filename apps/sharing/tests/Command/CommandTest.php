...
final class CommandTest extends AbstractSharingManagerTests {
	...
	public function testDefaultShareAccessContext(): void {
		foreach ($this->commandClasses as $class) {
			...
			$command = new $class(Server::get(ISharingManager::class));
	...
	protected function searchRecipients(ShareAccessContext $accessContext, ?array $recipientTypeClasses, string $query, int $limit, int $offset): array {
		...
		return ShareRecipient::formatMultiple($this->manager->searchRecipients($accessContext, $recipientTypeClasses, $query, $limit, $offset));
	}
	...
	protected function selectSharePermissionPreset(ShareAccessContext $accessContext, string $id, SharePermissionPreset $permissionPreset): array {
		$stdout = $this->runCommand(
			...
			[
				['id', $id],
				['permission-preset', $permissionPreset->value],
			],
			[],
		);
		/** @psalm-suppress MixedReturnStatement */
