...
final readonly class ShareRecipient {
	...
	public function format(bool $isUnique): array {
		if (($recipientType = (Server::get(ISharingRegistry::class)->getRecipientTypes()[$this->class] ?? null)) === null) {
		...
		return [
			...
			'secret' => $secret,
			'initiator' => $this->initiator?->format(),
		];
	}
	...
	public static function formatMultiple(array $recipients): array {
		$recipientTypes = Server::get(ISharingRegistry::class)->getRecipientTypes();
